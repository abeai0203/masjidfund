...
export default async function StateProjectsPage({ params }: { params: Promise<{ state: string }> }) {
  try {
    ...
    if (!resolvedParams?.state) {
      return (
        <div className="max-w-7xl mx-auto py-20 px-4 text-center">
          <h1 className="text-xl font-bold font-black text-foreground">Negeri tidak sah</h1>
          <Link href="/projects" className="text-primary mt-4 inline-block font-bold">Kembali ke semua wilayah</Link>
        </div>
      );
    }
      ...
      <div className="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8 w-full flex-grow flex flex-col">
        ...
        <div>
          <div className="mb-6 flex justify-between items-center text-sm text-foreground/60 font-medium tracking-wide">
            <span>MENUNJUKKAN {projects?.length || 0} PROJEK</span>
          </div>
    ...
    return (
      <div className="max-w-7xl mx-auto py-20 px-4 text-center">
        <h1 className="text-xl font-bold font-black text-foreground">Maaf, ralat berlaku semasa memuatkan data.</h1>
        <p className="text-foreground/60 mt-2">Sila cuba sebentar lagi atau semak wilayah lain.</p>
        <Link href="/projects" className="text-primary mt-6 inline-block font-bold">Kembali ke senarai projek</Link>
      </div>
    );
  }
}
